...
​		通过vue-element-admin单独封装一个上传表格、解析表格内容的功能组件，全局引入组件；设置对应的路由；

​		在需要导入表格的组件中引入该组件，然后使用该组件的*onSuccess()*方法并接收该组件处理过后的表格数据（results, header）；

​		因为后端需要的数据对象的键名不支持中文，所以还要通过一个映射关系将传入表格数据的中文键名转换为英文键名（键名其实就是表头）；

​		最后调用上传API发送请求，发送成功后进行提示；
